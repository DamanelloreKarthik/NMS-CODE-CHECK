...
from app.flows.parser import normalize_flow   # ✅ ADD THIS
...
UDP_IP = "0.0.0.0"
UDP_PORT = 4739   # IANA standard for IPFIX
BUFFER_SIZE = 65535

# Store templates per exporter + observation domain
templates_ipfix = {}
...
def parse_template_set(data, exporter_ip, domain_id):
    ...
    while offset + 4 <= len(data):
        ...
        templates_ipfix[(exporter_ip, domain_id, template_id)] = fields
...
def parse_data_set(template_id, data, exporter_ip, domain_id):
    key = (exporter_ip, domain_id, template_id)

    if key not in templates_ipfix:
        logger.warning(
            f"No IPFIX template {template_id} from {exporter_ip}"
        )
        return

    fields = templates_ipfix[key]
