...
import logging
import os
from app.path_analysis.intelligence import (
    detect_network_anomaly,
    classify_network_issue,
    calculate_path_stability
)
...
def build_dynamic_destination_candidates(db, source_ip: str):
...
def get_top_talkers(db, source_ip=None, minutes=60, limit=3):
    try:
        ...
        if not rows:
            logger.warning("Top talkers fallback triggered | source_ip=%s", source_ip)
...
def format_path_response(raw_data: dict, db, device_id: int):
    hops = raw_data["hops"]

    valid_latencies = [
        h["latency_avg"] for h in hops if h["latency_avg"] is not None
    ]
    ...
    baseline = get_latency_baseline(db, device_id, raw_data["destination_ip"])
    ...
    issue_type = classify_network_issue(hops)
    ...
    return {
        "summary": {
            ...
            "anomaly_detected": anomaly_data["is_anomaly"],
            "anomaly_score": anomaly_data["score"],
            "anomaly_reasons": anomaly_data["reasons"],
...
def analyze_network_path_with_context(
    db,
    source_ip: str,
    destination_ip: str,
    ...
    formatted_path = format_path_response(
        traceroute_data,
        db,
        device_id
    )
    ...
    if previous and "path" in previous:
        route_changed = detect_route_change(
            previous["path"],
            formatted_path["path"]
        )

    stability = calculate_path_stability(
        formatted_path["path"],
        [h.get("path", []) for h in history if "path" in h]
    )

    formatted_path["summary"]["stability_score"] = stability
...
def get_latency_baseline(db, device_id: int, destination_ip: str, window=60):
    past_runs = (
        db.query(PathRun)
        .filter(
            PathRun.device_id == device_id,
            PathRun.destination_ip == destination_ip,
            PathRun.executed_at >= datetime.utcnow() - timedelta(minutes=window)
        )
        .all()
    )
    ...
    for run in past_runs:
        ...
        for h in hops:
            if h.latency_avg:
                latencies.append(h.latency_avg)
...
def detect_route_change(previous_hops, current_hops):
    prev_ips = [h["ip_address"] for h in previous_hops if h["ip_address"]]
    curr_ips = [h["ip_address"] for h in current_hops if h["ip_address"]]

    return prev_ips != curr_ips