...
def base_query(db, start_time, end_time, event_source=None, interface=None):
    query = db.query(NetFlowRecord).filter(
        NetFlowRecord.flow_start.between(start_time, end_time)
    )
...
def get_top_talkers(query, source_ip=None, limit=3):

    filtered = query

    if source_ip:
        filtered = filtered.filter(NetFlowRecord.src_ip == source_ip)
